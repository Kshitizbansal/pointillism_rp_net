...
def boxes_iou3d_gpu(boxes_a, boxes_b):
    ...
    # height overlap
    boxes_a_height_min = (boxes_a[:, 2] - boxes_a[:, 3]).view(-1, 1)
    boxes_a_height_max = boxes_a[:, 2].view(-1, 1)
    boxes_b_height_min = (boxes_b[:, 2] - boxes_b[:, 3]).view(1, -1)
    ...
    ar_a = (boxes_a[:, 4] * boxes_a[:, 5]).view(-1, 1)
    ar_b = (boxes_b[:, 4] * boxes_b[:, 5]).view(1, -1)
    
    iou2d = overlaps_bev / torch.clamp(ar_a + ar_b - overlaps_bev, min=1e-7) 
    iou3d = overlaps_3d / torch.clamp(vol_a + vol_b - overlaps_3d, min=1e-7)

    return iou3d,iou2d

def boxes_iou3d_gpu_orig(boxes_a, boxes_b):
    """
    :param boxes_a: (N, 7) [x, y, z, h, w, l, ry]
    :param boxes_b: (M, 7) [x, y, z, h, w, l, ry]
    :return:
        ans_iou: (M, N)
    """
    boxes_a_bev = kitti_utils.boxes3d_to_bev_torch_orig(boxes_a)
    boxes_b_bev = kitti_utils.boxes3d_to_bev_torch_orig(boxes_b)
